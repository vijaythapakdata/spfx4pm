...
//handle search
  ...
  const filteredItems=items.filter((item)=>(item?.EmailAddress?.toLowerCase().includes(searchtext.toLowerCase())||item?.Age?.toString().includes(searchtext)||item?.Admin?.toLowerCase().includes(searchtext.toLowerCase())||item?.City?.toLowerCase().includes(searchtext.toLowerCase()))
||item?.Name?.toLowerCase().includes(searchtext.toLowerCase())
)
  return(
    <>

    <Input placeholder='Search here' style={{marginBottom:20,width:300}} value={searchtext} onChange={handleSearch}/>
    <Table columns={columns} dataSource={filteredItems}  pagination={{pageSize:5}}/>
    </>
  )
}
